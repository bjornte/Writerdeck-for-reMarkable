...
new2c_stat = (
    ...
    '                    vaultBeginUnlock("Enter private PIN to open this note")\n'
    '                    return\n'
    '                }\n'
    '                if (xhr.status !== 200) {\n'
    '                    console.log("doLoad failed status " + xhr.status)\n'
    '                    return\n'
    '                }\n'
    '                var response = sanitizeLoadedNote(xhr.responseText)'
)
...
new4b = (
    ...
    '                else { lobbyGoPage(1); openNotePicker() }\n'
    '            } else isOmni = !isOmni\n'
    '            event.accepted = true\n'
    '        }'
)
...
new6 = (
    ...
    '    property bool lobbyVaultLocked: true\n'
...
new7 = (
    ...
    '            })\n'
    '        }\n'
    '        if (lobbyFilesIndex >= lobbyNotesModel.count)\n'
    ...
    '        if (row.encrypted && lobbyVaultLocked) {\n'
    '            vaultPendingLoad = row.name\n'
    '            vaultBeginUnlock("Unlock to edit encrypted note"); return }\n'
    ...
    '            vaultBeginUnlock("Unlock to read encrypted note"); return }\n'
    '        isLobby = false\n'
    '        if (mode == 1) doc = query.text\n'
    '        saveFile()\n'
    '        lobbyOpenInReadMode = true\n'
    ...
    '            writerdeck.renameNote(oldName, name)\n'
    '            lobbyFilesMode = ""\n'
    '            lobbyFilesInput = ""\n'
    '        } else if (lobbyFilesMode === "new-encrypted") {\n'
    '            if (lobbyVaultLocked) { vaultBeginUnlock("Unlock to create encrypted note"); return }\n'
    ...
    '        if (lobbyNotesModel.count === 0) return\n'
    '        if (lobbyVaultLocked) { vaultBeginUnlock("Unlock to encrypt note"); return }\n'
    '        var row = lobbyNotesModel.get(lobbyFilesIndex)\n'
    '        if (!row || row.encrypted) return\n'
    '        writerdeck.encryptNote(row.name)\n'
    '    }\n'
    '\n'
    '    function lobbyDecryptSelected() {\n'
    '        if (lobbyNotesModel.count === 0) return\n'
    '        if (lobbyVaultLocked) { vaultBeginUnlock("Unlock to decrypt note"); return }\n'
    '        var row = lobbyNotesModel.get(lobbyFilesIndex)\n'
    '        if (!row || !row.encrypted) return\n'
    '        writerdeck.decryptNote(row.name)\n'
    '    }\n'
    '\n'
    ...
    '    }\n'
    '\n'
    '    function vaultBeginUnlock(reason) {\n'
    '        vaultPinInput = ""\n'
    '        vaultPinPending = ""\n'
    '        vaultOverlayReason = reason || ""\n'
    '        vaultOverlayMode = "unlock"\n'
    '    }\n'
    '\n'
    ...
    '    }\n'
    '\n'
    '    function requestVaultUnlock(reason, name) {\n'
    ...
    '            return\n'
    '        }\n'
    '        if (vaultOverlayMode === "unlock") {\n'
    '            writerdeck.unlockVault(vaultPinInput)\n'
    ...
    '                lobbyFilesInput = lobbyFilesInput.slice(0, -1)\n'
    '                return true\n'
    '            }\n'
    '            if (event.text && event.text.length === 1 && event.modifiers === Qt.NoModifier) {\n'
    '                lobbyFilesInput += event.text\n'
    '                return true\n'
    '            }\n'
    '            return true\n'
    '        }\n'
    ...
    '            return true\n'
    '        }\n'
    '        if (lobbyPage === 4 && lobbySettingsMode === "") {\n'
    ...
    '        }\n'
    '        if (lobbyPage === 1 && lobbyFilesMode === "" && lobbyEncryptionEnabled) {\n'
...
new7n_fn = (
    ...
    '        var col = pos - lineStartPos(pos, text)\n'
    '        var nextStart = nl + 1\n'
    '        var nextEnd = lineEndPos(nextStart, text)\n'
    ...
    '        var prevEnd = lineEndPos(prevStart, text)\n'
    ...
    '        var anchor\n'
    '        if (query.selectionStart === query.selectionEnd) {\n'
    '            fromPos = query.cursorPosition\n'
    '            anchor = query.cursorPosition\n'
    '        } else if (down) {\n'
    '            fromPos = Math.max(query.selectionStart, query.selectionEnd)\n'
    '            anchor = Math.min(query.selectionStart, query.selectionEnd)\n'
    '        } else {\n'
    '            fromPos = Math.min(query.selectionStart, query.selectionEnd)\n'
    '            anchor = Math.max(query.selectionStart, query.selectionEnd)\n'
    '        }\n'
    '        var newPos = down ? lineDownPos(fromPos, text) : lineUpPos(fromPos, text)\n'
    '        query.select(Math.min(anchor, newPos), Math.max(anchor, newPos))\n'
    '    }\n'
    '\n'
    ...
    '                var leftFrom = (query.selectionStart === query.selectionEnd)\n'
    '                    ? pos : Math.min(query.selectionStart, query.selectionEnd)\n'
    '                newPos = Math.max(0, leftFrom - 1)\n'
    '            } else if (event.key === Qt.Key_Right) {\n'
    '                var rightFrom = (query.selectionStart === query.selectionEnd)\n'
    '                    ? pos : Math.max(query.selectionStart, query.selectionEnd)\n'
    '                newPos = Math.min(text.length, rightFrom + 1)\n'
...
new7q = (
    ...
    '            suppressNextHomeKey = false\n'
    '        if (vaultOverlayMode !== "") {\n'
    '            if (event.key === Qt.Key_Escape) { vaultNumpadCancel(); event.accepted = true; return }\n'
    '            if (event.key === Qt.Key_Return) { vaultNumpadSubmit(); event.accepted = true; return }\n'
    '            if (event.key === Qt.Key_Backspace) { vaultNumpadTap("Bksp"); event.accepted = true; return }\n'
    '            if (event.key >= Qt.Key_0 && event.key <= Qt.Key_9) {\n'
    '                if (vaultHandleDigitKey(event.key - Qt.Key_0)) { event.accepted = true; return }\n'
    '            }\n'
    '            event.accepted = true\n'
    '            return\n'
    '        }\n'
