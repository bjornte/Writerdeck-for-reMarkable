...
new7n_fn = (
    ...
    '            var lo = pos\n'
    '            var hi = pos\n'
    '            if (key === Qt.Key_Right || key === Qt.Key_Down || key === Qt.Key_End) hi = text.length\n'
    '            else if (key === Qt.Key_Left || key === Qt.Key_Up || key === Qt.Key_Home) lo = 0\n'
    '            else return\n'
    '            query.select(Math.min(lo, hi), Math.max(lo, hi))\n'
    '            cursorStrong = true\n'
    '            cursorTimer.stop()\n'
    '            return\n'
    '        }\n'
    ...
    '                if (query.selectionStart !== query.selectionEnd\n'
    '                        && query.cursorPosition === head && head < anchor\n'
    '                        && lastShiftHorizKey === Qt.Key_Right)\n'
    '                    newPos = Math.min(anchor, head + 1)\n'
    '                else\n'
    '                    newPos = Math.max(0, head - 1)\n'
    '                cursorStrong = true\n'
    '                cursorTimer.stop()\n'
    '                query.select(newPos, anchor)\n'
    '                lastShiftHorizKey = Qt.Key_Left\n'
    '                event.accepted = true\n'
    '                return true\n'
    ...
    '                    ? pos : Math.max(query.selectionStart, query.selectionEnd)\n'
    '                if (query.selectionStart !== query.selectionEnd\n'
    '                        && query.cursorPosition === headR && headR > anchorR\n'
    '                        && lastShiftHorizKey === Qt.Key_Left)\n'
    '                    newPos = Math.max(anchorR, headR - 1)\n'
    '                else\n'
