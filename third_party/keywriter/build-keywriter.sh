...
new7n_fn = (
    ...
    '        } else if (shift && !cmd && !alt) {\n'
        '            if (event.key === Qt.Key_Left || event.key === Qt.Key_Right) {\n'
    '                // Shift+arrows: keep the opposite selection end as the anchor.\n'
    '                // Active end is the caret (query.cursorPosition).\n'
    '                var head = query.cursorPosition\n'
    '                var anchorH\n'
    '                if (query.selectionStart === query.selectionEnd) {\n'
    '                    anchorH = head\n'
    '                } else {\n'
    '                    anchorH = (query.cursorPosition === query.selectionStart)\n'
    '                        ? query.selectionEnd\n'
    '                        : query.selectionStart\n'
    '                }\n'
    '                var newHead = (event.key === Qt.Key_Left)\n'
    '                    ? Math.max(0, head - 1)\n'
    '                    : Math.min(text.length, head + 1)\n'
    '                cursorStrong = true\n'
    '                cursorTimer.stop()\n'
    '                query.select(anchorH, newHead)\n'
    ...
    '        var end = pos\n'
    '        if (cmd && !alt) {\n'
    '            // Ctrl+Backspace: delete the newline if caret is just before it.\n'
