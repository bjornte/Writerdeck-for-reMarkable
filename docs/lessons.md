...
Lobby look, wording, language packs, and Ctrl-letter chords live in `/home/root/.Writerdeck/lobby-ui.json` and `lobby-ui-i18n/`. Writerdeck reloads when those change (file/directory watch plus a short mtime poll — rename replaces often miss inotify on the tablet). Start that poll only after QApplication exists; a timer started from a static constructed before the app never fires. Bad or unreadable JSON keeps the last good load (or the baked-in defaults if nothing has loaded yet) — fix the file rather than restarting for a blank Lobby. If an old English `strings` block remains in `lobby-ui.json`, it overrides the language pack — clear it to `{}` unless you intend pinned overrides. Deploy seeds missing `lobby-ui.json` / packs only; new `visual` keys (for example `settingsLandscapeScrollGutter`) must be added by hand on an existing tablet file, or the embedded default applies until you set them. The phone page reads the same `language` over `GET /api/phone-ui` — hard-refresh after changing it.
