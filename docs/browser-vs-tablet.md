...
Lobby is driven by the keyboard: focus returns after touch, and every main control has a chord (Shortcuts tab). Borders, help text, dialog copy, and which letter each Ctrl-chord uses come from `/home/root/.Writerdeck/lobby-ui.json` on the tablet ([decisions.md](decisions.md) §36) — not from the phone. Private PIN on the tablet accepts USB digits and digits forwarded from the phone. Without a USB keyboard or an open phone/laptop page, Edit / New / Rename show a short connect tip (QR for the phone URL). An open page counts after WebSocket `hello`; Cursor’s embedded browser is excluded (User-Agent `Cursor/` or `Electron/`), so agent tabs do not hide the tip.
