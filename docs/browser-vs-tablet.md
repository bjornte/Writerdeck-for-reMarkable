...
List notes in the Files tab (tab 1; Home welcome screen is tab 6); create via New or Ctrl-K; open for edit via Edit, Enter, double-tap, or second tap on the selected row; open for read via Read or `v` (preview on e-ink); toggle edit and preview with Esc; rename and delete with `r` and `d`; reading font and PIN length in Settings (tab 4); rotate with Ctrl-R or Ctrl-arrows; Exit Writerdeck in Settings (confirm with Enter); sync status and Sync now on the Sync tab; USB keyboard layout in the Keyboard tab (`us` / Norwegian `no`, device verified); type from USB (Qt evdev qmap) or Bluetooth (same WebSocket path as the phone). Home from edit returns to Files with the last-edited note selected. Launch Lobby: Esc, L+R page buttons, `wd`, `~/wd`.
