...
Modifier+arrow and selection behaviour are tested on the device via `daemon/cmd/edit-harness` and `scripts/test-keyboard-harness.sh`, not by reading saved note bytes. Writerdeck publishes cursor/selection over the socket; the server exposes `GET /api/test/editor-state` and `POST /api/test/reset` (hard quit). Scenarios send keys over `/ws` like the phone UI. **90 scenarios**; sign-off **90/90 PASS** with `--fast`. Scoreboard: [editor-testing/milestone-runs.md](editor-testing/milestone-runs.md).
