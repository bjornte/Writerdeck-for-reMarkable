...
Editor behavior now lives in the Writerdeck-keywriter fork (decision 3). Residual risk is the tiny glue left in `build-keywriter.sh` and any future upstream merge conflicts — not a growing patch-script stack. Harness **110/110/0** remains product sign-off.