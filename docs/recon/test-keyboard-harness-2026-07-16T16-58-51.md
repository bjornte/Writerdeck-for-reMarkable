# Keyboard harness results

Run: 2026-07-16T16:58:52+02:00

Target: `192.168.1.8:8000`

Mode: sandbox-prepare (single session)

Timing: fast pauses

Summary: 15 pass, 0 fail, 0 prepare fail; total 17.0s

| Name | Result | Time (s) | Recovery | Cascade | Comments |
|------|--------|----------|----------|---------|----------|
| wrap-down-one-visual-line | pass | 1.3 | no | — | — |
| wrap-down-not-jump-paragraph | pass | 1.1 | no | — | — |
| wrap-up-from-visual-line-2 | pass | 1.6 | no | — | — |
| wrap-shift-down-one-visual | pass | 1.3 | no | — | — |
| wrap-shift-down-then-up-shrinks | pass | 1.2 | no | — | — |
| wrap-down-last-visual-line | pass | 1.2 | no | — | — |
| wrap-shift-down-last-to-eof | pass | 0.8 | no | — | — |
| wrap-mixed-newline-and-wrap | pass | 0.7 | no | — | — |
| wrap-down-goal-column | pass | 1.4 | no | — | — |
| wrap-combo-alt-left-word | pass | 1.2 | no | — | — |
| wrap-combo-alt-right-word | pass | 1.3 | no | — | — |
| wrap-combo-ctrl-bs-line | pass | 0.8 | no | — | — |
| wrap-shift-left-across-wrap | pass | 1.3 | no | — | — |
| wrap-home-on-visual-line | pass | 0.9 | no | — | — |
| wrap-end-on-visual-line | pass | 0.9 | no | — | — |
