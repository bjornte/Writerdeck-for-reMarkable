...
**Caveat — browser-scoped destructive ops (by design).** `ghDelete` and rename-then-repush only fire from the phone browser. External deletes/renames are handled by the marker-aware reconciler path (device-verified) — see [decisions.md](decisions.md) #19.
