...
- Settings screen (phone). A **Settings** overlay (font, PIN, display rotate) and a separate **Sync** overlay (GitHub sync) in the top bar; both dismiss via Done, ×, ESC, or backdrop click. Font choice (Inter / Literata / EB Garamond / DejaVu, pushed live to the editor) and PIN length persist to `/home/root/.Writerdeck/settings.json`. When sync is on and a repo is set, the Sync panel links to `github.com/{repo}`. A missing token shows a yellow banner below the status bar.
