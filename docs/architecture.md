...
- GitHub note-sync (optional, off by default). **Writerdeck-server** is the sync engine (GitHub Contents API); the phone POSTs the PAT to `/api/sync/token` and the server holds it **in RAM** for the service lifetime — never on disk. Settings hold non-secret `syncOn`/`syncRepo` and per-note `syncMeta` (SHA + fingerprint). Reconcile runs on boot, 3 min poll, Home/power exit, tablet CRUD, phone CRUD, and manual `POST /api/sync/run`. Same integrity contract: edit-lease gate, empty-push guard, copy-on-clash (#19, #24). Plan: [server-sync-implementation.md](server-sync-implementation.md).
