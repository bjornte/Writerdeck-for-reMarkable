...
The server stays up under the stock UI. Boot leaves xochitl on screen; open Writerdeck with page buttons, USB Esc, or `rmlobby`. Home from Lobby brings xochitl back; the phone can still reach port 8000. The PIN is shown in the Lobby. Lobby → Keyboard lists Bluetooth (phone URL, PIN, QR) then USB layout (US / Norwegian / Spanish / German / French), with live `(connected)` / `(not connected)` on each headline while that tab is open. Settings in landscape keeps a right scroll gutter (`visual.settingsLandscapeScrollGutter`) so fingers can flick without hitting buttons. Sync is optional and change-driven (boot, app open, document open, wake, Wi-Fi up, Home, power sleep — not a timer); the token never hits disk ([server-sync-implementation.md](server-sync-implementation.md)).
