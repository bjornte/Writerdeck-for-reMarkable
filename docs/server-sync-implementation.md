...
The reconcile engine runs in Writerdeck-server (Go). The phone handles token entry and sync settings only. The tablet Lobby Sync tab shows last sync, Sync now when configured, TOKEN NEEDED when RAM is empty, and SYNC OFFLINE / SYNC FAILED when Wi-Fi or GitHub errors apply.
...
Reconcile runs on boot, app open, wake, Wi-Fi up, Home, power sleep, CRUD, manual Sync, and token verify — not on a timer. Lobby document open also pulls that one file before load. Home, power sleep, and token verify skip GitHub when nothing local needs a push. Boot, app open, wake, Wi-Fi up, phone Sync, and Lobby Sync always list GitHub so remote-only edits still pull. Unchanged files are never PUT (including vault secrets), so clean runs do not create empty commits. Extra browser tabs that re-POST the same token do not start another reconcile. Journal lines use a short story form (`sync: …`); clean skips coalesce (see below). Power sleep runs reconcile on the server before suspend — the browser no longer waits. Wake and Wi-Fi return check GitHub again. Phone `sync.js` keeps drift UX only; no GitHub fetch calls remain.
...
Manual Sync with nothing dirty still hits GitHub once and logs `sync: checked GitHub (manual) — still nothing to change`. Boot, app open, wake, and Wi-Fi up do the same with their reason tags. Extra browser tabs that re-POST the same token do not start another reconcile.
...
Token POST then service restart — token cleared from RAM, browser auto-repost via `needtoken` (journal: `sync: checked GitHub (boot)` or a push/pull line when files move). Token survives Wi-Fi IP change (re-enter once per browser origin if IP changes). Edit on tablet, exit Home — GitHub updated within one reconcile when the note is dirty. Edit on GitHub, open the note from Lobby (or Sync now) — tablet updated if file not open in editor. Both edit same note — `(tablet copy).md` plus primary. Empty tablet, non-empty GitHub — pull restore, no junk copy. Tablet delete, rename, create — GitHub reflects without phone. Open note during reconcile — that file skipped, others sync. Power sleep — reconcile completes (or skips clean), no browser hang; wake checks GitHub again. Wi-Fi returns while Lobby is up — `(wifi)` reconcile. Sync off — no GitHub traffic. Bad or expired token — error banner, local saves continue. Clean tablet overnight — no timer, no empty commits; remote checks on boot, app open, wake, Wi-Fi, or Sync now; local edits sync on Home, power sleep, CRUD, or Sync now.
