...
- No jailbreak; preserve OTA (over-the-air firmware updates) => avoid Toltec.
- No on-device runtime deps => static Go binary (`CGO_ENABLED=0`, `GOOS=linux GOARCH=arm GOARM=7`).
- Markdown is the save format.
- Writerdeck-server cross-compiles on the Mac (the only device-reachable host) — `GOOS=linux GOARCH=arm GOARM=7 CGO_ENABLED=0`. Only Writerdeck needs GitHub’s Docker build (our keywriter fork + Toltec Qt libraries); don't build it on a host toolchain.
...
- Prune dead files as paths close — not just prose. When a phase finishes or a path is declared closed-failed, delete the machinery that only served it (one-off scripts, generated logs, build helpers, committed binaries) — but capture the durable lesson in TODO/DONE first. Git history is the archive: keep the gravestone (why it failed), drop the tooling. Before deleting, grep for references and fix link rot; never remove a file a live script still calls. Regenerable outputs (e.g. `docs/recon/` logs) are always safe to drop — keep the folder via `.gitkeep`.
