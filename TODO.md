...
8. Fork wrap-up hygiene (fresh session) — stop Writerdeck CI from surgically inserting `edit_mac_helpers.qml.inc` and concatenating Lobby; own assembly in [Writerdeck-keywriter](https://github.com/bjornte/Writerdeck-keywriter) (`qml.qrc` / in-tree include or a single committed `main.qml`). Then shrink `third_party/keywriter/build-keywriter.sh` to clone + assert + build. Verify: edit-session, critical **38/38/0**, full **110/110/0**. Ship tip documented in fork README (`6a15e08`).
