...
- [ ] Regression test for the ghost-note pull fix in `daemon/syncengine.go` `pullNote()`. Context: a "ghost note" is a note that still exists on GitHub but was removed from the tablet's filesystem *without* going through the app's delete path, so its stale marker in `SyncMeta` (`settings.json`) was never cleared. The old fast-path `if meta.SHA == gh.SHA { return false, nil }` matched that stale SHA and suppressed the re-pull forever (the GitHub blob SHA never changes), so the note never came back to the tablet. This bit `z-test-openafter.md` (on GitHub, missing from the device). Fix now only takes the fast-path when the file actually exists locally: `if _, ok := readNoteBytes(name); ok { return false, nil }`, otherwise it falls through and rewrites the file. Test to add in `daemon/syncengine_test.go`: seed meta so `meta.SHA == gh.SHA`, ensure NO local file on disk, call `pullNote(name)`, and assert it returns `(true, nil)` and writes the file (i.e. the ghost note is restored); add a sibling case where the local file DOES exist and assert it still returns `(false, nil)` (no needless rewrite / no regression). Cover both plain `.md` and encrypted `.md.enc` names since `readNoteBytes` handles both. No Go toolchain on the notes-repo machine, so build/`go test ./daemon` must be run on the dev machine.
